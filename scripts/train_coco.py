...
parser.add_argument("--w_lil", default=0.3, type=float, help="w_lil")
...
parser.add_argument("--update_prototype", default=6000, type=int, help="begin to update prototypes")
parser.add_argument("--cam2mask", default=50000, type=int, help="use mask from last layer")
parser.add_argument("--seg_mask_source", default=12000, type=int, help="use cam or cam_aux to supervise seg")
...
### dataset utils
parser.add_argument("--img_folder", default='/HOME/scz0658/run/Jaye_Files/Data/MSCOCO/JPEGImages', type=str, help="dataset folder")
parser.add_argument("--label_folder", default='/HOME/scz0658/run/Jaye_Files/Data/MSCOCO/SegmentationClass', type=str, help="dataset folder")
...
def train(args=None):
    ...
    for n_iter in range(args.max_iters):
        ...
        seg_cam_source = valid_cam if n_iter >= args.seg_mask_source else valid_cam_aux
        refined_pseudo_label = refine_cams_with_bkg_v2(par, inputs_denorm, cams=seg_cam_source, cls_labels=cls_label,  high_thre=args.high_thre, low_thre=args.low_thre, ignore_index=args.ignore_index, img_box=img_box, )
        ...
        # warmup
        if n_iter <= 8000:
            loss = 1.0 * cls_loss + 1.0 * cls_loss_aux + 0.0 * ptc_loss + 0.0 * lig_loss + 0.0 * lil_loss + 0.0 * seg_loss + 0.0 * reg_loss
        elif n_iter <= 12000:
